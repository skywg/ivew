    ...
    <div class="index">
        ...
        <div class="main">
            <div class="wrapper-container"  >
                <Form ref="govInfo" :model="govInfo" :rules="ruleInline" >
                <div style="width: 700px;margin: 40px auto">                   
                    <div style="text-align: center" v-if="willShow">
                        <h1  style="margin-bottom: 20px;font-size: 22px;font-weight: normal">机关注册</h1>
                        <span>为确保您的信息准确，每一步都必须填写</span>
                            <Form-item prop="gov_name" class="input">
                                <Input v-model="govInfo.gov_name" placeholder="机关全称" style="width: 260px;height: 25px"  size="large"></Input>
                            </Form-item>
                            <Form-item prop="address" class="input">
                                <Input v-model="govInfo.address" placeholder="注册地址" style="width: 260px;height: 25px" size="large"></Input>
                            </Form-item>
                            <Form-item prop="legal_person" class="input">
                                <Input v-model="govInfo.legal_person" placeholder="法人姓名" style="width: 260px;height: 25px" size="large"></Input>
                            </Form-item>
                            <Form-item prop="gov_code" class="input">
                                <Input v-model="govInfo.gov_code" placeholder="注册号" style="width: 260px;height: 25px" size="large"></Input>
                            </Form-item>
                            <Form-item prop="organization_code" class="input">
                                <Input v-model="govInfo.organization_code" placeholder="组织机构代码" style="width: 260px;height: 25px" size="large"></Input>
                            </Form-item>
                            <Form-item class="input">
                                <div><h2 style="font-size: 22px;margin-top: 20px;margin-bottom: 20px;font-weight: normal">请上传事业单位法人证明</h2></div>
                                <div>
                                 ...
                                 <div>
                                    <Button type="primary" shape="circle" class="last-btn" @click="submit">提交</Button>
								</div>
						    </Form-item>                                                                  
						</div>  
					</div>
...
<script>
    ...
    export default {
        data () {
            let govPhone=(rules,value,cb)=> {　//转换函数，主要目的是传给store内方法的参数。
                this.$store.dispatch('govPhone', {rules, value, cb})//这儿的checkAge是写在store中的，vuex规定参数必须传对象。
            }
            return {
                ...
                ruleInline: {
                    gov_name:[
                        {validator:govPhone, required: true, message: '请填写企业名称', trigger: 'blur'}
                    ],
                    user: [
        ...
        methods: {
            ...
            submit(){
                ...
                axios({
                    method: 'post',
                    url: 'http://localhost:1111/govInfo/saveGov',
                    data:{
                        gov_name:this.govInfo.gov_name,
                        address:this.govInfo.address,
                        legal_person:this.govInfo.legal_person,
                        gov_code:this.govInfo.gov_code,
                        organization_code:this.govInfo.organization_code,
                    }
                }).then(response => {
