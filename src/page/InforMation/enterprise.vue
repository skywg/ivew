...
<template>
	<div>
		<Row :gutter="6" style="margin-top: 40px">
			<Col span="6"  class="qiye-box"  style=" padding: 0;" v-for="e in enterprise">
			<div>
				<img src="../../img/qiye-img.png" alt="">

				<p style="font-size: 14px;text-align: center;padding: 0 8px"><router-link to="/enterprise/detail">{{e.corpName}}</router-link></p>

			</div>
			</Col>
		</Row>
		<Button type="ghost" class="new-more" @click="enterprises()">更多</Button>
	</div>
</template>
<script>
    ...
    export default {
        ...
        methods:{
            show:function() {
                axios.get('http://localhost:1111/corpInfo/findPage/' + this.currentPage + "?pageSize=" + this.pageSize)
                    .then(response => {
            ...
            enterprises(){
                this.$router.push({
                    path: '/pro/enterpriseList',query:{flag: 6}
                })
            }
        }
    }
