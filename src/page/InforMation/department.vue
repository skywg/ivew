...
<template>
	<div>
		<Row :gutter="16" style="margin-bottom: 200px;margin-top: 40px;margin-left: 3px">
			<Col span="6" class="qiye-box" style=" padding: 0;" v-for="d in depart">

			<div>
				<img src="../../img/qiye-img.png" alt="">
				<p style="font-size: 14px;text-align: center"><router-link to="/departmentDetail">{{d.govName}}</router-link></p>
			</div>
			</Col>
			<Button type="ghost" class="new-more" @click="department()">更多</Button>
		</Row>
            ...
            nextPage(page) {
                ...
                axios.get('http://localhost:1111/govInfo/find/' + this.currentPage + "?pageSize=" + this.pageSize)
                    .then(response => {
                        this.depart = response.data.list;
                        this.total = response.data.total;
                        console.log("机关="+this.depart);
                    })
            },
            department() {
                this.$router.push({
                    path: '/pro/enterpriseList',query:{flag: 7}
                })
            }
        }
    }
