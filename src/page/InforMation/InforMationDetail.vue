            ...
            <div class="searchList">
                <div style="margin-bottom: 30px;">
                    <Breadcrumb separator=">">
                        ...
                        <Breadcrumb-item href="/pro/inforMationList?flag=1">资讯</Breadcrumb-item>
                ...
                <Row>
                    <i-col class="listLeft">
                        ...
                        <div >
                            ...
                            <div  @click="jubaoClick = true" class="jubao">
                                举报
                            </div>
                        ...
                        <div class="marginTop">
                            <Row>
                                ...
                                <i-col span="21" class="loginTip" v-if="user == null">
                                    <div>
                                        <Button type="success"  class="btnReset" @click="loginuser()">登录</Button>后发表评论
                                        <Modal v-model="modal2" width="360" :closable="false">
                                        <Tabs class="loginTab" v-model="values">
                        ...
                        <div class="CommentTab">
                            ...
                            <Tabs value="name1">
                                <Tab-pane label="最新评论" name="name1">
                                    <ul class="userComment-list" v-if="list != null">
                                    ...
                                    <ul class="userComment-list"  v-else>
                                        <li>
                                            <div class="comment">
                                                <span><img src="../../img/huifu-icon.png"/>回复</span>
                                            </div>
                                            <div  v-if="user != null">
                                                <Input  v-model="huifu" type="textarea"  placeholder="请输入..." style="width: 100%;margin: 30px 0px 10px 0;"></Input>
                                                <Button type="primary" size="small" style="float:right;margin-bottom:20px" @click="reply(0)">发送</Button>
                                            </div>
                                        </li>
                                    </ul>
                                </Tab-pane>
...
export default {
    ...
    methods: {
        fetchData: function () {
            axios.get('http://localhost:1111/inforMation/findInforMationDetail?id=' + this.$route.query.id)
                .then(response => {
        ...
        findComment() {
            axios.get('http://localhost:1111/inforMation/findInfoComment/' + this.currentPage + '?id=' + this.$route.query.id + '&pageSize=' + this.pageSize)
                .then(response => {
        ...
        myComment() {
            if(this.user){
                axios.get('http://localhost:1111/inforMation/findMyComment/' + this.currentPage + '?id=' + this.$route.query.id + '&pageSize=' + this.pageSize)
                    .then(response => {
        ...
        upvote() {
            axios.get('http://localhost:1111/inforMation/upvote?id=' + this.$route.query.id)
                .then(response => {
        ...
        reply(id) {
            axios({
                method: 'post',
                url: 'http://localhost:1111/inforMation/saveHuiFu',
                data: {
                    postId: id,
        ...
        dianzan(id) {
            axios.get('http://localhost:1111/inforMation/saveZan?id=' + id)
                .then(response => {
        ...
        goToPrevious(id){
            axios.get('http://localhost:1111/inforMation/findInforMationDetail?id=' + id)
                .then(response => {
                    this.content = response.data.content
                    this.title = response.data.title
                    this.createTime = response.data.createTime
