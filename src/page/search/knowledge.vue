            ...
            <div class="new-last" v-if="data.list.list!=''">
                <ul>
                    <li class="new-last1" style="position: relative"  v-for="item in data.list.list">
                        ...
                        <div class="text-min" ><p>稻是一年生自花授粉禾本科植物。按其生物学特性分为籼稻、粳稻、糯稻，按物之一...</p></div>
                        <div class="text-min2">
...
<script>
    ...
    export default {
        ...
        methods: {
           /* fetchData: function () {
                this.list = []
                axios.get('http://localhost:1111/knowLege/findKnowLedgeTitle/' + this.currentPage + "?title=" + this.$route.query.title)
                    .then(response => {
            ...
            nextPage(val){
                ...
                this.list = []
                axios.get('http://localhost:1111/knowLege/findKnowLedgeTitle/' + this.currentPage + "?title=" + this.$route.query.title +"&pageSize=" +this.pageSize)
                    .then(response => {
