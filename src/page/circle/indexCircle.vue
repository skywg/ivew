			...
			<div class="container container2">
				...
				<Row>
					...
					<div v-for="(value,key) in circle">
						...
						<Row>
							<Col span="8" style="margin-bottom: 20px" v-for="v in value">
							<div>
...
export default {
	...
	methods: {
		//最新帖子
		articles() {
			axios.get('http://localhost:1111/CoterieArticle/query/')
				.then(response => {
		...
		//本周最热
		weeks() {
			axios.get('http://localhost:1111/CoterieArticle/weekHot/')
				.then(response => {
		...
		//圈子
		circles() {
			axios.get('http://localhost:1111/CoterieArticle/circle/')
				.then(response => {
		...
		//排行榜
		ranks() {
			axios.get('http://localhost:1111/CoterieArticle/rank/')
				.then(response => {
